...
var FSHADER_SOURCE = `
    ...
    void main(){
        ...
        else if(distance(texColor0, vec3(1.0, 0.0, 0.0) ) < 0.3 && distance(texColor2, vec3(1.0, 1.0, 1.0) ) < 0.3) {
...
function draw() {
    ...
    for (let i = 0; i < objComponents.length; i++) {
        ...
        initAttributeVariable(
            gl,
            program.a_Position,
            objComponents[i].vertexBuffer
        );
        initAttributeVariable(
            gl,
            program.a_TexCoord,
            objComponents[i].texCoordBuffer
        );
        initAttributeVariable(
            gl,
            program.a_Normal,
            objComponents[i].normalBuffer
        );
