...
function initVertexBufferForLaterUse(
    ...
    if (tagents != null)
        o.tagentsBuffer = initArrayBufferForLaterUse(
            gl,
            new Float32Array(tagents),
            3,
            gl.FLOAT
        );
    if (bitagents != null)
        o.bitagentsBuffer = initArrayBufferForLaterUse(
            gl,
            new Float32Array(bitagents),
            3,
            gl.FLOAT
        );
    if (crossTexCoords != null)
        o.crossTexCoordsBuffer = initArrayBufferForLaterUse(
            gl,
            new Float32Array(crossTexCoords),
            1,
            gl.FLOAT
        );
...
async function main() {
    ...
    sonicObj = await loadOBJtoCreateVBO("gun.obj");
...
function drawOneRegularObject(
    ...
    for (let i = 0; i < obj.length; i++) {
        ...
        initAttributeVariable(
            gl,
            program.a_crossTexCoord,
            obj[i].crossTexCoordsBuffer
        );
